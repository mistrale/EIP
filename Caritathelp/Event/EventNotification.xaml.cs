...
namespace Caritathelp.Event
{
    ...
    /// </summary>
    public sealed partial class EventNotification : Page
    {
        private EventModel events;
        private string responseString;
        private NotificationRequest eventNotif;
        ...
        public async void getNoficationMemberShip()
        {
            ...
            try
            {
                ...
                eventNotif = JsonConvert.DeserializeObject<NotificationRequest>(responseString);
                if (eventNotif.status != 200)
                {
                }
                else
                {
                    ...
                    for (int i = 0; i < eventNotif.response.Count; i++)
                    {
                        if (eventNotif.response[i].notif_type.Equals("JoinEvent", StringComparison.Ordinal))
                        {
                            ...
                            title.Content = eventNotif.response[i].sender_name;
        ...
        private async void addUserToEvent(object sender, RoutedEventArgs e)
        {
            Button button = sender as Button;
            ...
            try
            {
                ...
                string url = (Global.API_IRL + "/guests/reply_guest");
        ...
        private async void refuseUserToEvent(object sender, RoutedEventArgs e)
        {
            Button button = sender as Button;
