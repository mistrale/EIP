...
{
    public sealed partial class VolunteerEvenement : Page
    {
        private string id;
        ...
        private async void getEvent()
        {
            ...
            try
            {
                var template = new UriTemplate("http://api.caritathelp.me/volunteers/" + id + "/events" + "{?token}");
                template.AddParameter("token", (string)Windows.Storage.ApplicationData.Current.LocalSettings.Values["token"]);
                var uri = template.Resolve();
                Debug.WriteLine(uri);
                ...
                else
                {
                    ...
                    for (int x = 0; x < events.response.Count; ++x)
                    {
                        ...
                        rights.Text = events.response[x].rights + " - " + events.response[x].begin;
